...
public class ResourceManager implements IResourceManager
{
	...
	protected String execute(Vector<String> command) {
		...
		return Utils.execute(this,command);
	}
...
//			writeData(xid, customer.getKey(), customer);
	...
	public int getItemPrice(int xid, String key, int quantity) {
		// Check if the item is available
		ReservableItem item = (ReservableItem)readData(xid, key);
		if (item == null)
		{
	...
	{
		String result = "Below is the analytics for " +getName()+ " type\n";
		for (String key: m_data.keySet()) {
			int quantity = 	queryNum(xid, key);
			if (quantity <= bound){
				result += key + " has only " + quantity + " remaining\n";
			}
