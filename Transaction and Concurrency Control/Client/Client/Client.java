...
public abstract class Client
{
	...
	public void start()
	{
		...
		while (true)
		{
			...
			try {
				...
				System.err.println((char)27 + "[31;1mCommand exception: " + (char)27 + "[0m " +
						"\nThis is an invalid transaction\nIt may not exist at all or has been committed" +
	...
	public void execute(Command cmd, Vector<String> arguments) throws RemoteException, NumberFormatException, InvalidTransactionException, TransactionAbortedException
	{
		switch (cmd)
		{
			...
			case AnalyticsFlight: {
				...
				System.out.println("Analytics of flight [xid=" + arguments.elementAt(1) + "]");
				System.out.println("Remaining seats less than: " + arguments.elementAt(2));

				int id = toInt(arguments.elementAt(1));
				int quantity = toInt(arguments.elementAt(2));

				String seats = m_resourceManager.analyticsFlight(id, quantity);
				System.out.println("Flights with number of seats less than "+ quantity+": " + seats);
			...
			case AnalyticsRoom: {
				...
				System.out.println("Analytics of Rooms [xid=" + arguments.elementAt(1) + "]");
				System.out.println("Remaining quantity less than: " + arguments.elementAt(2));

				int id = toInt(arguments.elementAt(1));
				int quantity = toInt(arguments.elementAt(2));

				String seats = m_resourceManager.analyticsRoom(id, quantity);
				System.out.println("Rooms with remaining quantity less than "+ quantity+": " + seats);
			...
			case AnalyticsCar: {
				...
				System.out.println("Analytics of Cars [xid=" + arguments.elementAt(1) + "]");
				System.out.println("Remaining quantity less than: " + arguments.elementAt(2));

				int id = toInt(arguments.elementAt(1));
				int quantity = toInt(arguments.elementAt(2));

				String seats = m_resourceManager.analyticsCar(id, quantity);
				System.out.println("Cars with remaining quantity less than "+ quantity+": " + seats);
			...
			case ReserveFlight: {
				...
				if (m_resourceManager.reserveFlight(id, customerID, flightNum) != null) {
			...
			case ReserveCar: {
				...
				if (m_resourceManager.reserveCar(id, customerID, location) != null) {
			...
			case ReserveRoom: {
				...
				if (m_resourceManager.reserveRoom(id, customerID, location) != null) {
			...
			/*case Analytics:{
				checkArgumentsCount(2, arguments.size());
				System.out.println("Showing the analytics [xid=" + arguments.elementAt(1) + "]");
				int id = toInt(arguments.elementAt(1));
				String result = m_resourceManager.analytics(id);
				if (result != null){
			...
			case Summary:{
				checkArgumentsCount(2, arguments.size());
				System.out.println("Showing the summary [xid=" + arguments.elementAt(1) + "]");
				int id = toInt(arguments.elementAt(1));
				String result = m_resourceManager.summary(id);
				if (result != null){
					System.out.println(result);
			...
			case Start: {
				...
				System.out.println("The xid for this transaction is " + xid);
				break;
			}
			case Commit: {
				checkArgumentsCount(2, arguments.size());
				int xid = toInt(arguments.elementAt(1));
				System.out.println("Trying to commit the transaction xid " + xid);
				boolean hasSucceed = m_resourceManager.commit(xid);
				if (hasSucceed)
					System.out.println("The transaction " + xid + " was committed successfully");
				else
					System.out.println("The transaction " + xid + " was aborted from server side");
				break;
			}
			case Abort: {
				checkArgumentsCount(2, arguments.size());
				int xid = toInt(arguments.elementAt(1));
				System.out.println("Trying to abort the transaction xid " + xid);
				m_resourceManager.abort(xid);
				System.out.println("The transaction " + xid + " was aborted successfully");
				break;
			}
			case Shutdown: {
				checkArgumentsCount(1, arguments.size());
				System.out.println("Trying to shutdown the whole system (including the servers)");
				m_resourceManager.shutdown();
				System.out.println("Successfully shutdown the servers and middleware");
				break;
			}
			case Quit:
				checkArgumentsCount(1, arguments.size());
	...
	public static boolean toBoolean(String string)// throws Exception
	{
		...
		if ("y".equals(string) || "yes".equals(string)){
			return true;
		}
		if ("1".equals(string)){
			return true;
